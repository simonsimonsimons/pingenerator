...
export default async function handler(req, res) {
  ...
  try {
    ...
    // --- Schritt A: Hintergrundbild generieren ---
    ...
    // --- Schritt B: Text mit der neuen Schriftart auf das Bild schreiben ---
    const image = await Jimp.read(imageBuffer);
    
    // NEU: Lädt die schwarze Schriftart
    const fontPath = path.join(process.cwd(), 'fonts', 'open-sans-64-black.fnt');
    const font = await Jimp.loadFont(fontPath);
    
    const text = `Top 10 Geschenke für:\n${anlass}`;
    
    // VEREINFACHT: Schreibt den Text nur einmal in Schwarz
    image.print(
      font, 
      0, // x
      0, // y
